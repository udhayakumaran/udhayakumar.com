export const profile = {
  name: "Udhaya Kumar",
  primaryTitle: "Staff Backend Engineer",
  positioning:
    "I own backend and data platforms from system design through migration, production operation, and recovery. Experienced in direct customer partnership — designing architectures for merchant engineering teams, conducting technical deep-dives on custom requirements, and guiding production implementations.",
  experience: "13+ years",
  location: "India",
  availability: "Open to Indian, international, remote, and hybrid opportunities",
  email: "mail4udhaya@gmail.com",
  phone: "+91 7259948358",
  links: {
    linkedin: "https://linkedin.com/in/udhayakumark",
    github: "https://github.com/udhayakumaran",
  },
  convertCart: {
    tenure: "Aug 2020 – May 2026",
    platformOwnership: "5+ years (Aug 2020 – May 2026)",
    merchants: "200+",
    commercePlatforms: "5",
    microservices: "50+",
    teamGrowth: "4 → 7 engineers",
    freshness: "Days → sub-minute for webhooks",
  },
  proof: {
    merchants: "200+ merchants across five commerce platforms",
    freshness: "Customer and order data freshness: sub-5-minute for webhook platforms, ~1–6 hours for others (vs. days-late baseline)",
    identityResolution: "Deterministic identity resolution across email, device ID, cookie, phone, and platform customer IDs on a legacy, distributed profile system — used across ~200-300 merchants; matched ~15%+ of contacts into an existing identity.",
    segmentation: "1,000+ regularly-trained active segments (used continuously) within six months of launch",
    recommendations: "80% of all merchants using recommendation blocks (adopted from launch)",
    recommendationsLatency: "200-400ms p99 (measured; target 500ms p99 across five regions)",
    mysql: "3+ minutes → <500ms (360x improvement) — fixed a pagination/join cardinality inversion in a recurring reporting query",
    dataIncident: "50M-row CDC snapshot consumed 60% CPU and blocked production writes",
    clickhouseMigration: "Reduced customer segmentation cost from $2,470/month (BigQuery) to $850/month (ClickHouse Scale on GCP) — a 66% reduction, verified from invoices. cityHash64-based batched hashing reduced compute 4x (64GB → 16GB, minimum ClickHouse Cloud Production limit).",
    segmentationCSMTimeSaved: "CSM time saved: ~15 hours/week on manual segment creation",
  },
  technologies: [
    "Node.js",
    "TypeScript",
    "MySQL",
    "Debezium",
    "Google Pub/Sub",
    "MongoDB",
    "BigQuery",
    "ClickHouse",
    "Redis",
    "GKE",
  ],
  roleFit: [
    "Staff Backend Engineer",
    "Staff Platform Engineer",
    "Principal Engineer",
    "Founding Engineer",
    "Forward Deployed Engineer (FDE)",
    "Senior FDE / Field Engineer",
    "Solutions Engineer",
    "Senior Backend Engineer",
    "Senior Platform Engineer",
    "Senior Data Infrastructure Engineer",
  ],
  resume: { label: "Staff Backend Engineer | Debezium · ClickHouse · Pub/Sub · Real-time CDC", file: "/Udhayakumar-Staff-Backend-Engineer-Resume.pdf", description: "13+ years backend infrastructure, data platforms, and production systems" },
  experienceTimeline: [
    { index: "A1a", years: "2020–2021", company: "ConvertCart", role: "Senior Software Developer - Team Lead — partnered with merchant teams on custom requirements; diagnosed fragile cron-sync system, redesigned to real-time CDC pipelines, stopped production crisis, initiated recommendation engine" },
    { index: "A1b", years: "2021–2026", company: "ConvertCart", role: "Senior Product Development Lead — partnered directly with 200+ merchant engineering teams; designed architectures for customer data systems; conducted technical deep-dives on integration patterns, production deployment, migration strategies; grew platform team 4→7" },
    { index: "A2b", years: "2019–2020", company: "Independent Consultant", role: "Backend Consultant — partnered with clients on custom backend requirements; conducted technical design sessions, prototyping, production implementation guidance" },
    { index: "A2", years: "2018–2019", company: "Friday Media Group", role: "Software Development Lead — marketplace APIs, content ingestion, data pipeline; managed 3–6 developers" },
    { index: "A3", years: "2014–2018", company: "Scientific Games", role: "Senior Software Engineer — query optimization (3+ min → <500ms via join reordering), reporting integrations, microservices" },
    { index: "A4", years: "2012–2014", company: "Tenlegs", role: "Web Developer — CSS-to-inline email template converter, web applications" },
    { index: "A5", years: "2010–2012", company: "ISPG Technologies", role: "Junior Software Engineer — LAMP-stack web projects, CMS, social-networking platforms" },
  ],
} as const;

export const roleLanes = [
  "Senior Software Engineer — Backend",
  "Senior Backend Engineer — Platform / Infrastructure",
  "Senior Platform Engineer",
  "Senior Data Platform Engineer",
  "Senior Data Infrastructure Engineer",
  "Technical Lead",
  "Staff Backend Engineer",
  "Founding Engineer",
];

